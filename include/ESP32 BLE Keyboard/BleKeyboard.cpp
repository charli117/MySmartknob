...
void BleKeyboard::begin(void)
{
  ...
  inputDialKeys =hid->inputReport(DIAL_KEYS_ID);
...
void BleKeyboard::sendDialReport(uint8_t keys)
{
    ...
    if (this->isConnected())
  {
    this->inputDialKeys->setValue((uint8_t*)dial_report, sizeof(dial_report));
    this->inputDialKeys->notify();
#if defined(USE_NIMBLE)        
    //vTaskDelay(delayTicks);
    this->delay_ms(_delay_ms);
#endif // USE_NIMBLE
...
void BleKeyboard::onConnect(BLEServer* pServer) {
  ...
  desc->setNotifications(true);
  desc = (BLE2902*)this->inputDialKeys->getDescriptorByUUID(BLEUUID((uint16_t)0x2902));
  desc->setNotifications(true);
