...
static const float ZERO_ELECTRICAL_OFFSET = 1.59;
...
void MotorTask::run() {
    ...
    while (1) {
        ...
        // 检查旋钮是否“空闲”（一段时间内没有移动）并靠近定位中心，如果是，它会慢慢调整参考系，使旋钮被认为正好在定位的中间（以避免不断地为电机供电，试图纠正那个轻微的错误）
        ...
        // 计算要施加的扭矩，并根据角度调整当前状态（例如，如果旋转超过棘爪，则增加当前位置）
...
int MotorTask::getPosition(void) {
    return config.position;
}

int MotorTask::getDeviceType(void) {
    return config.device_type;
}

int MotorTask::getDeviceOperate(void) {
    return config.device_operate;
}
